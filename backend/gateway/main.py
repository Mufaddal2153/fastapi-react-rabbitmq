...
async def registeration(user_data:UserRegisteration):
    try:
        print("API: ", f"{AUTH_BASE_URL}/api/users")
...
async def reset_password(user_data:UserCredentials):
    try:
        response = requests.post(f"{AUTH_BASE_URL}/api/users/reset_password", json={"username":user_data.username,"password":user_data.password})
        if response.status_code == 200:
            return response.json()
        else:
            raise HTTPException(status_code=response.status_code, detail=response.json())
...
async def ocr(file: UploadFile = File(...),
        Authorization: str = _fastapi.Header(None),
