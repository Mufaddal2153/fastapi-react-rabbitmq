...
async def generate_token(
    ...
    check_user = await _services.get_user_by_email(email=user_data.username, db=db)
    if not check_user:
        logging.info('User with that email does not exist')
        raise _fastapi.HTTPException(
            status_code=404,
...
async def reset_password(user_data: _schemas.GenerateUserToken, db: _orm.Session = _fastapi.Depends(_services.get_db)):
    user = await _services.get_user_by_email(email=user_data.username, db=db)
    if not user:
        logging.info('User with that email does not exist')
        raise _fastapi.HTTPException(
            status_code=404,
    ...
    user.hashed_password = _services.hash_password(user_data.password)
    db.add(user)
    db.commit()
