...
def hash_password(password):
    # Hash a password
    return _bcrypt.hashpw(password=password.encode('utf-8'), salt=_bcrypt.gensalt()).decode('utf-8')
...
async def create_user(user: _schemas.UserCreate, db: _orm.Session):
    ...
    user_obj = _models.User(email=email, name=name, hashed_password=hash_password(user.password))
    db.add(user_obj)
    db.commit()
    db.refresh(user_obj)
    return user_obj
